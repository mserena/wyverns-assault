...
{
	float currentRelativeTime = getRelativeTimePosition();
	
	updateTimePosition( elapsedSeconds );
	
	float nextRelativeTime = getRelativeTimePosition();

	updateTriggers( currentRelativeTime, nextRelativeTime );
}

void ExampleDummyAnimation::updateTimePosition( const float elapsedSeconds )
{
	...
	m_timePosition += ( elapsedSeconds * m_speed );
	if ( getLoop() )
	{
		if ( m_timePosition < 0 )
		{
