...
#include "ExampleDummyAnimation.h"
...
void ExampleAnimationSystem::loadAnimationTree( const tecnofreak::property::Property* animationInfo )
{
	...
	for ( int i = 0; i < getNodeCount(); i++ )
	{
		...
		if ( parameterNode )
		{
			...
			if ( ! parameter )
			{
				throw std::exception( "missing parameter" );
			}
...
void ExampleAnimationSystem::update( const float elapsedSeconds )
{
	getSharedData()->setElapsedSeconds( elapsedSeconds );
	getSharedData()->increaseUpdateId();

	disableCurrentAnimations();

	getReachedAnimationNodes().clear();
	getOutputNode()->evaluateAnimations( getReachedAnimationNodes() );
	
	flattenReachedAnimations();
	updateCurrentAnimations( elapsedSeconds );
}

void ExampleAnimationSystem::disableCurrentAnimations()
{
...
void ExampleAnimationSystem::parseTriggerInfo( ExampleAnimation* animation, const Property* triggerInfo )
{
	...
	Trigger* trigger = new Trigger( triggerName, relativeTime, minWeightThreshold );
