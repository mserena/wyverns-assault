#include "..\..\include\GUI\GuiObjectTextDisplay.h"

using namespace WyvernsAssault;

GuiObjectTextDisplay::GuiObjectTextDisplay(const Ogre::MovableObject* p, const Ogre::Camera* c) {
	m_p = p;
	m_c = c;
	m_enabled = false;
	m_text = "";
...
GuiObjectTextDisplay::~GuiObjectTextDisplay() {
...
void GuiObjectTextDisplay::enable(bool enable) {
	m_enabled = enable;
	if (enable)
		m_pOverlay->show();
	else
...
void GuiObjectTextDisplay::setText(const Ogre::String& text) {
	m_text = text;
	m_pText->setCaption(m_text);
}

void GuiObjectTextDisplay::update()  {
	if (!m_enabled)
		return;
